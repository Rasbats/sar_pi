...
= SAR_pi Manual - OpenCPN

This document is about the 'sar_pi' plugin, which we will refer to as SAR_pi  in this manual.
...
== Goal
SAR_pi generates search patterns for SAR missions and saves them as GPX files for importing into OpenCPN.

image::sar_liferaft.jpeg[]

== How to install
- From the plugin catalogue in OpenCPN  (more information below)
- By manually compiling the source code from Github and importing the plugin in OpenCPN (more information in the README.md)
...
* Forum:
https://www.cruisersforum.com/forums/f134/sar-plugin-233875-13.html[SAR Plugin Thread]
...
== Contributions
The plugin was developed by https://github.com/SaltyPaws[SaltyPaws], based on the external software https://sites.google.com/site/navigationalalgorithms/sar-search-patterns[SAR Search patterns] by Andres Ruiz González.

https://github.com/Rasbats[Rasbats] has updated the plugin, following advice from an Australian SAR Operations Centre Coordinator, Stephen Hancock.

https://github.com/rgleason[RGleason] has contributed ideas for the GUI. 

https://github.com/Kupofty[Kupofty] has contributed to GUI updates, bug fixes and code refactoring.
...
https://maritimesafetyinnovationlab.org/wp-content/uploads/2021/02/Doc.9731-EN-IAMSAR-Manual-International-Aeronautical-and-Maritime-Search-and-Rescue-Manual-Volume-III-Mobile-Facilities.pdf
...
https://web.archive.org/web/20210916104627/http://www.marine-knowledge.com/marine-navigation/search-and-rescue-patterns/

This last is the only reference that we have found for the 'Quadrant Search (Oil Rig)'
search (may take some time to load the page as it is a Wayback Machine archive).
...
follow in order to perform an efficient search and rescue operation.
...
It is very important that your search operation is as efficient as
...
Note: in most circumstances SAR operations are coordinated by the Coast
...
If you want the search pattern to start at your *ship's* position you go to the bottom half of the window and click “Copy ship position”.
...
We save the file in a folder where it can easily be found. For instance
in a folder “SAR”.
...
* *PS* : Parallel Search
* *SS* : Expanding Square Search (Square Search)
* *VS* : Sector Search (Vector Search)
* *QS* : Quadrant Search (Oil Rig)
...
the route. The speed you choose for the search will be used for the route calculations and will appear in the route manager.
...
Square”, “Sector Search” or “Quadrant Search (Oil Rig)”.
...
The search pattern will show up as a new route. Make sure it is
“visible” (the “eye” should be open). You can inspect the properties of
the route. The speed you choose for the search will be used for the route calculations. 
...
== Setting up the "Quadrant Search (Oil Rig)" pattern.

Click the *SAR icon* in the Toolbar

Choose the desired search pattern. In this case: *Quadrant Search (Oil Rig)*
