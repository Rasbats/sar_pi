...
void Dlg::ConvertToDegree()
{
    ...
    case 0:
        ...
        if (m_Nship->GetCurrentSelection() == 0) {
            m_bitmap_trackln1->SetBitmap(*_img_trackln1);
        } else if (m_Nship->GetCurrentSelection() == 1) {
...
*/
{
    ...
    if (this->m_Nship->GetCurrentSelection() == 0 && Pattern == 1)
        wxMessageBox(
    ...
    if (write_file) {
        wxFileDialog dlg(this, _("Export SAR track GPX file as"), wxEmptyString,
            defaultFileName, _T("GPX files (*.gpx)|*.gpx|All files (*.*)|*.*"),
    ...
    */
    ...
    if (!user_canceled && !error_occurred) {
        ...
        case 1: {
            ...
            // if (leg_distancex < 0.1) { leg_distancex = 1.0; }//check for
            // negative or small values
            if (nlegs < 0.1) {
                nlegs = 1.0;
            ...
            */
            ...
            case 0: // IAMSAR single unit
            {
                ...
                if (write_file) {
                    Addpoint(Route, wxString::Format(wxT("%f"), lat1),
                        wxString::Format(wxT("%f"), lon1), _T("CSP"),
                        _T("diamond"), _T("WPT"));
                }
                ...
                for (int x = 1; x <= nlegs;
                    ...
                    for (int y = 0; y <= 1; y++) { // Loop over the tracks
                        ...
                        destRhumb(
                            lat1, lon1, -ESheading, ESdistance, &lati, &loni);
                        ...
                        if (write_file) {
                            Addpoint(Route, wxString::Format(wxT("%f"), lati),
                                wxString::Format(wxT("%f"), loni), wpt_title,
                                wpt_mark, _T("WPT"));
                        }
                ...
                this->m_Distance->SetValue(
                    wxString::Format(wxT("%g"), SAR_distance));
                this->m_Time->SetValue(
                    wxString::Format(wxT("%g"), (SAR_distance / speed)));
                break;
            }
            ...
            case 1: // two units - iamsar
            {
                ...
                for (i = 1; i <= 2; i++) {
                    ...
                    if (i == 1) {
                        if (write_file) {
                            Addpoint(Route,
                                wxString::Format(wxT("%f"), latDatum),
                                wxString::Format(wxT("%f"), lonDatum),
                                _T("Datum"), _T("diamond"), _T("WPT"));
                        }
                        ESheading = approach - 90.0;
                        ESdistance = leg_distancey / 2;
                        destRhumb(latTemp, lonTemp, -ESheading, ESdistance,
                            &lati, &loni);
                    ...
                    if (i == 2) {
                        ...
                        if (write_file) {
                            Addpoint(Route2,
                                wxString::Format(wxT("%f"), latDatum),
                                wxString::Format(wxT("%f"), lonDatum), _T(""),
                                _T(""), _T("WPT"));
                        }
                        ESheading = approach + 90.0;
                        destRhumb(latTemp, lonTemp, -ESheading, ESdistance,
                        ...
                        if (write_file) {
                            Addpoint(Route2, wxString::Format(wxT("%f"), lati),
                                wxString::Format(wxT("%f"), loni), wpt_title,
                                wpt_mark, _T("WPT"));
                        }
                        lat1 = lati;
                    ...
                    for (int x = 1; x <= nlegs;
                        ...
                        for (int y = 0; y <= 1; y++) { // Loop over the tracks
                            ...
                            destRhumb(lat1, lon1, -ESheading, ESdistance, &lati,
                                &loni);
                            SAR_distance += ESdistance;
                            ...
                            if (i == 1) {
                                if (write_file) {
                                    Addpoint(Route,
                                        wxString::Format(wxT("%f"), lati),
                                        wxString::Format(wxT("%f"), loni),
                                        wpt_title, wpt_mark, _T("WPT"));
                                }
                            } else {
                                if (write_file) {
                                    Addpoint(Route2,
                ...
                if (write_file)
                    root->LinkEndChild(Route2);
                this->m_Distance->SetValue(
                    wxString::Format(wxT("%g"), SAR_distance / 2));
                this->m_Time->SetValue(
                    wxString::Format(wxT("%g"), (SAR_distance / speed) / 2));
                break;
            }
...
{
    this->m_Lat1->SetValue(
        wxString::Format(wxT("%.6f"), plugin->GetCursorLat()));
    this->m_Lon1->SetValue(
        wxString::Format(wxT("%.6f"), plugin->GetCursorLon()));
...
void Dlg::setDDMM()
{ // after entering dd.dddd from cursor, menu, lat
